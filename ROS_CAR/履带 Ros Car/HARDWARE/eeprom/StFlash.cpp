...
//添加一下简化函数
void STFLASH::writeBuf(char *buf,uint16_t leng){
	Write(FLASH_SAVE_ADDR,(u16*)buf,leng);
}
void STFLASH::readBuf(char *buf,uint16_t leng){
	Read(FLASH_SAVE_ADDR,(u16*)buf,leng);
}
/*
下载的时候，魔术棒里面别选择erase full chip （一般选erase sectors）的话一般代码不超过总的flash减去STM32_FLASH_EEPROM  的大小，下载代码也不会擦除模拟的eeprom
