...
void IIC_Init(void){			
  ...
  RCC_APB2PeriphClockCmd(IIC_GPIO_CLK, ENABLE); //使能PB端口时钟
  GPIO_InitStructure.GPIO_Pin = IIC_SDA_PIN|IIC_SCL_PIN;	//端口配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;      //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;     //50M
  GPIO_Init(IIC_GPIO_PORT, &GPIO_InitStructure);					      //根据设定参数初始化GPIOB 
}
